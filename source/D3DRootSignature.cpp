...
{
	// MSDN: "Specifying Root Signatures in HLSL"
	// https://learn.microsoft.com/en-us/windows/win32/direct3d12/specifying-root-signatures-in-hlsl

	template<typename T, typename U>
	bool IsFlagSet(T Input, U Flag)
	{
		return (Input & Flag) == Flag;
	}

	std::string BorderColorToString(D3D12_STATIC_BORDER_COLOR Color)
	{
		switch (Color)
		{
	...
	std::string RootDescriptorFlagsToString(D3D12_ROOT_DESCRIPTOR_FLAGS Flags)
	{
		...
		if (IsFlagSet(Flags, D3D12_ROOT_DESCRIPTOR_FLAG_DATA_VOLATILE))
			add("DATA_VOLATILE");

		if (IsFlagSet(Flags, D3D12_ROOT_DESCRIPTOR_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE))
			add("DATA_STATIC_WHILE_SET_AT_EXECUTE");

		if (IsFlagSet(Flags, D3D12_ROOT_DESCRIPTOR_FLAG_DATA_STATIC))
			add("DATA_STATIC");
	...
	std::string RootDescriptorRangeFlagsToString(D3D12_DESCRIPTOR_RANGE_FLAGS Flags)
	{
		...
		if (IsFlagSet(Flags, D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE))
			add("DESCRIPTORS_VOLATILE");

		if (IsFlagSet(Flags, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_VOLATILE))
			add("DATA_VOLATILE");

		if (IsFlagSet(Flags, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE))
			add("DATA_STATIC_WHILE_SET_AT_EXECUTE");

		if (IsFlagSet(Flags, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC))
			add("DATA_STATIC");

		if (IsFlagSet(Flags, D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_STATIC_KEEPING_BUFFER_BOUNDS_CHECKS))
	...
	std::string RootFlagsToString(D3D12_ROOT_SIGNATURE_FLAGS Flags)
	{
		...
		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT))
			add("ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_DENY_VERTEX_SHADER_ROOT_ACCESS))
			add("DENY_VERTEX_SHADER_ROOT_ACCESS");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_DENY_HULL_SHADER_ROOT_ACCESS))
			add("DENY_HULL_SHADER_ROOT_ACCESS");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_DENY_DOMAIN_SHADER_ROOT_ACCESS))
			add("DENY_DOMAIN_SHADER_ROOT_ACCESS");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_DENY_GEOMETRY_SHADER_ROOT_ACCESS))
			add("DENY_GEOMETRY_SHADER_ROOT_ACCESS");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_DENY_PIXEL_SHADER_ROOT_ACCESS))
			add("DENY_PIXEL_SHADER_ROOT_ACCESS");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_STREAM_OUTPUT))
			add("ALLOW_STREAM_OUTPUT");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_LOCAL_ROOT_SIGNATURE))
			add("LOCAL_ROOT_SIGNATURE");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_DENY_AMPLIFICATION_SHADER_ROOT_ACCESS))
			add("DENY_AMPLIFICATION_SHADER_ROOT_ACCESS");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_DENY_MESH_SHADER_ROOT_ACCESS))
			add("DENY_MESH_SHADER_ROOT_ACCESS");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_CBV_SRV_UAV_HEAP_DIRECTLY_INDEXED))
			add("CBV_SRV_UAV_HEAP_DIRECTLY_INDEXED");

		if (IsFlagSet(Flags, D3D12_ROOT_SIGNATURE_FLAG_SAMPLER_HEAP_DIRECTLY_INDEXED))
		...
		D3D12_SHADER_VISIBILITY Visibility)
	{
		...
		default:
			s += std::format("UNKNOWN_TYPE_{}(unk{}", static_cast<uint32_t>(Type), Param.ShaderRegister);
			break;
		}
	...
	std::string RootDescriptorTableToString(const D3D12_ROOT_DESCRIPTOR_TABLE1& Table, D3D12_SHADER_VISIBILITY Visibility)
	{
		...
		for (uint32_t i = 0; i < Table.NumDescriptorRanges; i++)
		{
			...
			default:
				s += std::format("UNKNOWN_TYPE_{}(unk{}", static_cast<uint32_t>(descriptor.RangeType), descriptor.BaseShaderRegister);
				break;
			}
	...
	std::string StaticSamplerToString(const D3D12_STATIC_SAMPLER_DESC& Sampler)
	{
		...
		if (Sampler.BorderColor != D3D12_STATIC_BORDER_COLOR_OPAQUE_WHITE)
			s += std::format(", borderColor={}", BorderColorToString(Sampler.BorderColor));
